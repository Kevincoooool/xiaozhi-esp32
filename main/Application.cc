...
extern const char p3_nishuo_start[] asm("_binary_nishuo_p3_start");
extern const char p3_nishuo_end[] asm("_binary_nishuo_p3_end");

extern const char p3_haode_start[] asm("_binary_haode_p3_start");
extern const char p3_haode_end[] asm("_binary_haode_p3_end");

extern const char p3_dakai_start[] asm("_binary_dakai_p3_start");
extern const char p3_dakai_end[] asm("_binary_dakai_p3_end");

extern const char p3_guanbi_start[] asm("_binary_guanbi_p3_start");
extern const char p3_guanbi_end[] asm("_binary_guanbi_p3_end");

extern const char p3_liangyidian_start[] asm("_binary_liangyidian_p3_start");
extern const char p3_liangyidian_end[] asm("_binary_liangyidian_p3_end");

extern const char p3_anyidian_start[] asm("_binary_anyidian_p3_start");
extern const char p3_anyidian_end[] asm("_binary_anyidian_p3_end");
...
void Application::uart_task(void)
{
    ...
    ESP_ERROR_CHECK(uart_set_pin(UART_NUM_1, 8, 41, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE));
    ...
    while (1)
    {
        ...
        if (recv_length)
        {
            ...
            int len = uart_read_bytes(UART_NUM_1, data, recv_length, 10 / portTICK_PERIOD_MS);
            // ESP_LOGI(TAG, "rec:%d ", len);
            ...
            if (data[2] == 0x00)
            {
                ...
                PlayLocalFile(p3_nishuo_start, p3_nishuo_end - p3_nishuo_start);
            ...
            {
                gpio_set_level(GPIO_NUM_17, 1);
                PlayLocalFile(p3_dakai_start, p3_dakai_end - p3_dakai_start);
            }
            else if (data[2] == 0x02)
            {
                gpio_set_level(GPIO_NUM_17, 1);
                PlayLocalFile(p3_guanbi_start, p3_guanbi_end - p3_guanbi_start);
            }
            else if (data[2] == 0x03)
            {
                gpio_set_level(GPIO_NUM_17, 1);
                PlayLocalFile(p3_liangyidian_start, p3_liangyidian_end - p3_liangyidian_start);
            }
            else if (data[2] == 0x06)
            {
                gpio_set_level(GPIO_NUM_17, 1);
                PlayLocalFile(p3_anyidian_start, p3_anyidian_end - p3_anyidian_start);
            }
            else if (data[2] == 0x05)
            {
                gpio_set_level(GPIO_NUM_17, 1);
                PlayLocalFile(p3_haode_start, p3_haode_end - p3_haode_start);
            }
            // else
            // pa_switch = 1;
        }
...
void Application::Start()
{
    ...
    xTaskCreate([](void *arg)
        ...
        app->uart_task();
        vTaskDelete(NULL); }, "uart_task", 1024 * 6, this, 5, NULL);
