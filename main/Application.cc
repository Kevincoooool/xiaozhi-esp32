...
BinaryProtocol *Application::AllocateBinaryProtocol(const uint8_t *payload, size_t payload_size)
{
    ...
    auto protocol = (BinaryProtocol *)malloc(sizeof(BinaryProtocol) + payload_size);
...
void Application::AudioEncodeTask()
{
    ...
    while (true)
    {
        std::unique_lock<std::mutex> lock(mutex_);
        cv_.wait(lock, [this]()
                 { return !audio_encode_queue_.empty() || (!audio_decode_queue_.empty() && audio_play_queue_.size() < 2); });
            ...
            if (type != NULL) {
                if (strcmp(type->valuestring, "tts") == 0) {
                    ...
                    if (text != NULL) {
                        ...
                        memset(ai_relpy, 0, sizeof(ai_relpy));
        strcpy(ai_relpy, text->valuestring);
                        label_ask_set_text(ai_relpy);
                    }
