...
public:
    ...
    void HandleNegotiateData(uint8_t* data, int len, uint8_t** output_data, 
        ...
        case SEC_TYPE_DH_PARAM_LEN:
            ...
            dh_param_ = static_cast<uint8_t*>(malloc(dh_param_len_));
            if (dh_param_ == nullptr) {
                return;
            }
            break;
        ...
        case SEC_TYPE_DH_PARAM_DATA:{
            ...
            int ret = mbedtls_dhm_read_params(&dhm_, &param, &param[dh_param_len_]);
            if (ret) {
                return;
            }
            ...
            ret = mbedtls_dhm_make_public(&dhm_, dhm_len, self_public_key_, dhm_len, myrand, nullptr);
            if (ret) {
                return;
            }
    
            ret = mbedtls_dhm_calc_secret(&dhm_, share_key_, SHARE_KEY_LEN, &share_len_, myrand, nullptr);
            if (ret) {
                return;
            }

            ret = mbedtls_md5(share_key_, share_len_, psk_);
            if (ret) {
                return;
            }
    
            ret = mbedtls_aes_setkey_enc(&aes_, psk_, 128);
            if (ret) {
                return;
            }

            *output_data = &self_public_key_[0];
            *output_len = dhm_len;
            *need_free = false;
            break;
            }
    ...
    int Encrypt(uint8_t iv8, uint8_t* crypt_data, int crypt_len) {
        ...
        int ret = mbedtls_aes_crypt_cfb128(&aes_, MBEDTLS_AES_ENCRYPT, crypt_len,
    ...
    int Decrypt(uint8_t iv8, uint8_t* crypt_data, int crypt_len) {
        ...
        int ret = mbedtls_aes_crypt_cfb128(&aes_, MBEDTLS_AES_DECRYPT, crypt_len,
            &iv_offset, iv0, crypt_data, crypt_data);
        if (ret) {
            return -1;
        }
