...
#define BSP_I2S_DUPLEX_MONO_CFG_AI(_sample_rate)                                                         \
    {                                                                                                 \
        .clk_cfg = I2S_STD_CLK_DEFAULT_CONFIG(_sample_rate),                                          \
...
static void i2s_example_write_task(void *args)
{
    ...
    //     ESP_ERROR_CHECK(i2s_channel_preload_data(i2s_tx_chan, w_buf, EXAMPLE_BUFF_SIZE, &w_bytes));
    // }
...
esp_err_t bsp_extra_audio_init(const i2s_std_config_t *i2s_config)
{
    ...
    /* Setup I2S channels */
    const i2s_std_config_t std_cfg_default = BSP_I2S_DUPLEX_MONO_CFG_AI(16000);
